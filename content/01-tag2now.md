...
- **관리 요소** 다수(ALB, 태스크 정의, 서비스, 관리형 DB) → **Docker Compose 하나**
- **비용** 고정 과금 → **free tier**
- **파이프라인** ①테스트와 빌드(**E2E 게이트** 통과 필수), ②ECR 푸시(OIDC keyless)는 재사용. **③배포 단계만 교체**
- **배포 실패 감지** `wait-for-service-stability`로 워크플로우 레벨에서 확인

운영 복잡도를 낮춘 대가로 가용성을 내줬다. 단일 노드(SPOF)는 서비스 규모에 맞춰 의도적으로 감수한 선택이다.
...
- **문제**: **게임 데이터를 가져올 공식 SDK와 문서 미존재.** 서버는 오픈소스이나 게임의 실제 호출 방식(요청 순서, 바이너리 헤더 규칙)은 어디에도 미기록.
...
- **문제**: 매칭 시작 시 해당 방이 목록에서 사라짐 → **조회 API만으로는 매칭 중인 유저 파악 불가**. 실제 활동 인원 전량 누락.
- **성과**: 직전 스냅샷과 **diff로 사라진 RANK_MATCH 방을 매칭 시작으로 판정**(재등장이나 TTL 만료로 해제) → **매칭 중 유저 수 정확히 집계**. 가상 방(Phantom Room)으로 실시간 노출.

### 3. 비동기 선언적 트랜잭션 데코레이터

- **문제**: 서비스 메서드마다 **세션 열기, 커밋, 롤백 코드 반복**. 정작 중요한 비즈니스 로직은 그 사이에 매몰.
- **성과**: Spring `@Transactional` 패턴을 Python async에 적용한 `@transactional` 데코레이터로 세션 관리 코드 제거. 예외 시 자동 롤백 보장.

### 4. Redis 공유 캐시: 같은 결과를 반복 조회하는 트래픽

- **문제**: **같은 화면인데 조회는 사람 수만큼 반복.** 매 요청마다 RPCN 재조회 시, 내 서버는 물론 **통제 불가한 외부 RPCN 서버까지 부하 전가 위험.**
- **성과**: 데이터 수명에 맞춘 **Redis TTL 캐시**(방 목록 10s, 리더보드 60s, 서버 목록 1h)로 중복 조회 제거. 캐시를 프로세스 밖에 배치해 **백엔드 다중 인스턴스에서도 캐시 공유**.
...
| AWS 인프라 이해 | ECR, ECS(Fargate), ALB, ElastiCache, Route53, CloudFront, IAM을 직접 구성하고 Lightsail로 옮기며 배포 경로 전체를 이해. OIDC keyless 배포로 귀결. |
