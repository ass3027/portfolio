...
| **History** | PostgreSQL 스냅샷 저장, 시간별·일별 통계 집계 |
...
- **문제**: 공식 SDK와 게임별 호출 명세 부재. 요청 순서와 바이너리 헤더 규칙 미기록.
- **성과**: 서버 목록, 방 검색, 리더보드, 스코어 조회 등 프로토콜 명령 7종 재현. 단위 테스트와 통합 테스트 기반 동작 검증.

### 2. 스냅샷 Diff 기반 매칭 탐지

- **문제**: 매칭 시작 시 방 목록에서 사라지는 구조. 조회 API만으로 매칭 중 유저 파악 불가.
- **성과**: 사라진 RANK_MATCH 방의 매칭 시작 판정, 재등장과 TTL 만료 기반 해제. 매칭 중 유저의 Phantom Room 실시간 노출.

### 3. Redis 공유 캐시로 외부 RPCN 부하 제어

- **문제**: 동일 화면의 사용자별 반복 조회. 통제할 수 없는 RPCN 서버까지 부하 전가 위험.
- **성과**: 방 목록 10초, 리더보드 60초, 서버 목록 1시간 TTL 캐시 적용. 다중 인스턴스 공유용 프로세스 외부 캐시 구성.
