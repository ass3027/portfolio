...
RPCN은 오픈소스지만 게임별 호출 방식은 문서화돼 있지 않고 공식 SDK도 없다. Rust 서버 실행과 분석으로 바이너리 프로토콜을 역공학했다. 단위 테스트와 통합 테스트로 검증한 Python 클라이언트로 실시간 대시보드를 구축했다.

### 핵심 목표

- **SDK 없는 RPCN 바이너리 프로토콜 역공학**: Rust 서버 직접 분석, 프로토콜 명령 7종 테스트 검증
...
### 1. RPCN 바이너리 프로토콜 역공학과 클라이언트 검증

- **문제**: 공식 SDK와 게임별 호출 명세 부재로 요청 순서와 바이너리 헤더 규칙 확인 불가.
- **해결**: Rust RPCN 서버 분석 기반 바이너리 프로토콜 역공학 및 Python 클라이언트 구현.
