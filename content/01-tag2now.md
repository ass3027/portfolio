...
AWS Lightsail 단일 서버 + Docker Compose로 실서비스 운영 중(`match.tag2now.click`). ECS 대비 비용을 낮추기 위해 규모에 맞춰 인프라를 다운스케일했다. **오픈 카카오톡방의 실사용자 피드백을 직접 받아 오류 수정과 기능 개선을 반복**하며 서비스를 유지하고 있다.
