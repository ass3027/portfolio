...
RPCN은 오픈소스지만 게임별 호출 방식은 문서화돼 있지 않고 공식 SDK도 없다. Rust 서버를 직접 실행하고 분석해 호출 프로토콜을 역공학하고, 단위 테스트와 통합 테스트로 검증한 Python 클라이언트로 실시간 대시보드를 구축했다.

### 핵심 목표

- **SDK 없는 RPCN 프로토콜 역공학**: Rust 서버 직접 분석, 프로토콜 명령 7종 테스트 검증
- **실시간 매칭 상태 시각화**: 스냅샷 Diff 기반 매칭 탐지와 Phantom Room 실시간 노출
- **AWS 서비스 조합 기반 실서비스 운영**: 인프라 구성, 자동 배포, 도메인 연결, 운영 비용 조정까지 직접 수행
...
| **History** | 스냅샷의 PostgreSQL 저장, 시간별, 일별 통계 집계 |
| **Community** | 게시판 CRUD, PostgreSQL과 DynamoDB 어댑터 교체 |

### 운영 현황

- **호스팅**: AWS Lightsail 단일 서버와 Docker Compose, `match.tag2now.click` 운영
- **비용 조정**: ECS 대비 비용 절감을 위한 서비스 규모 기반 다운스케일
...
- **문제**: 게임 데이터를 가져올 공식 SDK와 문서 부재. 요청 순서와 바이너리 헤더 규칙 미기록.
- **성과**: 서버 목록, 방 검색, 리더보드, 스코어 조회 등 프로토콜 명령 7종 재현. 단위 테스트와 통합 테스트 기반 동작 검증.

### 2. 스냅샷 Diff 기반 매칭 탐지

- **문제**: 매칭 시작 시 방이 목록에서 사라져 조회 API만으로 매칭 중 유저 파악 불가.
- **성과**: 사라진 RANK_MATCH 방을 매칭 시작으로 판정하고 재등장과 TTL 만료로 해제. 매칭 중 유저를 Phantom Room으로 실시간 노출.

### 3. Redis 공유 캐시로 외부 RPCN 부하 제어

- **문제**: 같은 화면을 사용자마다 반복 조회하면 애플리케이션뿐 아니라 통제할 수 없는 RPCN 서버에도 부하 전가.
- **성과**: 방 목록 10초, 리더보드 60초, 서버 목록 1시간 TTL 캐시 적용. 다중 인스턴스에서 공유 가능한 프로세스 외부 캐시 구성.
