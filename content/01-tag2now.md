...
RPCN은 오픈소스지만 게임별 호출 방식은 문서화돼 있지 않고 **공식 SDK도 없다**. Rust 서버 실행과 분석으로 **바이너리 프로토콜을 역공학**했다. 단위 테스트와 통합 테스트로 검증한 Python 클라이언트로 실시간 대시보드를 구축했다.

### 핵심 목표

- **SDK 없는 RPCN 바이너리 프로토콜 역공학**: 공식 SDK와 호출 명세 부재, 서버 구현이 유일한 근거
- **조회 API가 답하지 않는 매칭 상태 시각화**: 매칭 중인 방과 유저 모두 응답에 없음
- **개인 부담 가능한 비용으로 실서비스 운영**: 인프라 구성부터 배포, 도메인, 비용까지 1인 부담
...
```
     ...
     │ FastAPI Application                                │
     │ ┌────────────────────────────────────────────────┐ │
     │ │ RpcnClient  역공학한 바이너리 프로토콜, 명령 7종 │ │
     │ └────────────────────────────────────────────────┘ │
     │ ┌────────────┐ ┌────────────┐ ┌──────────────────┐ │
...
| 모듈 | 역할 |
|---|---|
| **RpcnClient** | 역공학한 바이너리 프로토콜 구현, TCP/TLS 세션과 명령 7종 |
...
- **문제**: 매칭 시작 시 방이 목록에서 사라지고, 조회 API는 존재하는 방만 보여줘 방을 조회 중인 유저 자체가 응답에 없음. 조회 API만으로는 매칭 중인 유저(생성자·조회자 모두) 파악 불가.
...
### 4. 실서비스 운영: 비용 다운스케일과 실사용자 피드백 반영

- **문제**: 서비스 규모 대비 ECS 기반 인프라의 고정 비용 과다, 실사용자 이슈 파악 채널 부재.
- **해결**: AWS Lightsail 단일 서버와 Docker Compose로 다운스케일, 오픈 카카오톡방으로 실사용자 제보 채널 운영.
- **성과**: `match.tag2now.click` 실서비스 운영과 free tier 수준 비용, 제보 기반 오류 수정·기능 개선 반영.
