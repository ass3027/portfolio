...
> 페이지 수는 잠정. 콘텐츠를 덜어내거나(포트폴리오는 스캔 자료) 합쳐서 **20쪽 내외**를 목표로 한다. "딥다이브 분량 자체를 줄인다"가 여백 대신 쓰는 정공법.

| # | 페이지 | 담을 내용 |
|---|--------|-----------|
| 1 | **표지 / About** | 프로필 카드(연락처·통계) + "이세진입니다" + bio + 핵심 기술 바 |
| 2 | **경력 & 자격증** | Work Experience(스피어에이엑스) + 자격증 그리드(6개) — 한 페이지에 균형 배치 |
| 3 | **프로젝트 개요** | 4개 카드(Tag2Now/KTX/FireWatcher/LiDAR)를 2×2로 배치 → LiDAR 고립 문제 해결 |
| 4 | **Tag2Now — 개요+기술스택** | 헤더 + 프로젝트 배경 + 압축 기술 스택(10 pill) |
| 5 | **Tag2Now — 아키텍처** | ascii 다이어그램 + 모듈 표 (architecture.png 플레이스홀더는 정리 — §7) |
| 6 | **Tag2Now — 인프라/CI·CD** | Before/After 비교 + CI/CD 파이프라인 |
| 7 | **Tag2Now — 핵심 도전** | 4-1~4-4 도전을 압축 배치 (필요시 2쪽) |
| 8 | **Tag2Now — 배운 점** | learn 카드 6개 |
| 9 | **KTX — 개요+기술스택** | 헤더 + 배경 + 기술 스택(9 pill) |
| 10 | **KTX — 아키텍처 결정** | Plan A/B 표 + 2단 일관성 ascii |
| 11 | **KTX — 핵심 도전** | 도전 1~4 (필요시 2쪽) |
| 12 | **KTX — SLO & Before/After** | SLO 표(S1~S6) + 실험 표(E1~E3) |
| 13 | **KTX — 배운 점** | learn 카드 |
| 14 | **FireWatcher — 개요+도전+배운점** | 분량 적어 1~2쪽으로 압축 |
| 15 | **LiDAR — 개요+도전+배운점** | 분량 적어 1~2쪽으로 압축 |

> **주의**: 위 매핑은 현재 콘텐츠 기준 추정. 실제로는 각 페이지에 넣어 보고 넘치면 쪼개고, 비면 합친다. Hemp는 제외.
...
## 7. 열린 결정 사항 (사용자와 확정 필요)

1. **분량**: 34쪽 → 목표 몇 쪽? (권장: 20쪽 내외, 딥다이브 압축.)
2. **화면 네비게이션**: 사이드바 유지(페이지 점프) / 페이지 인디케이터 / 순수 스크롤 중?
3. **모바일 처리**: `transform: scale`로 축소 미리보기 / 모바일 전용 간이 뷰 / 데스크톱 우선(모바일 최소 대응)?
4. **`architecture.png`**: 깨진 플레이스홀더 → (a) 실제 다이어그램(SVG/PNG) 제작해 삽입 / (b) 당분간 ASCII 박스만 두고 플레이스홀더 제거. (CLAUDE.md에 SVG/PNG 교체가 "예정"으로 기록됨.)
5. **페이지 비율**: A4 가로 고정 vs 16:9(화면 친화) — 제출이 PDF 인쇄라면 A4 가로가 정석.
