...
모든 사용자가 사실상 같은 화면(방 목록·매칭 현황)을 보기 때문에, 조회 요청은 사용자 수만큼 늘어나지만 응답 내용은 동일하다. 매 요청마다 RPCN을 다시 조회·집계하면 **같은 결과를 만드는 데만 부하가 몰린다.** TTL 캐시로 이 중복을 제거하고, 캐시를 애플리케이션 메모리가 아닌 **외부 Redis**에 둔 덕에 백엔드를 여러 인스턴스로 확장해도 인스턴스별로 캐시가 갈리지 않고 하나를 공유한다.
